...
class AvodModelBev(model.DetectionModel):
    ...
    def __init__(self, model_config, train_val_test, dataset):
        ...
        # Create the RpnModel
        self._rpn_model = RpnModelBev(model_config, train_val_test, dataset)
    ...
    def build(self):
        ...
        top_anchors = prediction_dict[RpnModelBev.PRED_TOP_ANCHORS]
        ground_plane = rpn_model.placeholders[RpnModelBev.PL_GROUND_PLANE]

        class_labels = rpn_model.placeholders[RpnModelBev.PL_LABEL_CLASSES]
        ...
        fc_input_rois = [bev_rois]
        #fc_input_rois = [bev_rois, img_rois]
        fc_input_weights = [bev_mask]
        #fc_input_weights = bev_mask, img_mask]
        fc_output_layers = \
            avod_fc_layers_builder.build(
                layers_config=avod_layers_config,
                input_rois=fc_input_rois,
                input_weights=fc_input_weights,
        ...
        anchors_gt = rpn_model.placeholders[RpnModelBev.PL_LABEL_ANCHORS]
        if self._box_rep in ['box_3d', 'box_4ca']:
            boxes_3d_gt = rpn_model.placeholders[RpnModelBev.PL_LABEL_BOXES_3D]
        ...
        with tf.variable_scope('avod_encode_mb_anchors'):
            ...
            if self._box_rep == 'box_3d':
                ...
                #top_anchors = tf.Print(top_anchors, [tf.shape(top_anchors)], message='print top anchors shape: ')
