...
class RpnModelBev(model.DetectionModel):
    ##############################
    ...
    def __init__(self, model_config, train_val_test, dataset):
        ...
        super(RpnModelBev, self).__init__(model_config)
        ...
        self._pred_anchors_dir = self.dataset.kitti_utils.pred_anchors_dir
        self._use_grid_anchors = self.dataset.kitti_utils.use_grid_anchors
        ...
        #use_grid_anchors = True
        #use_grid_anchors = False
        use_grid_anchors = self._use_grid_anchors
        if use_grid_anchors:
            print('[rpn models.py] use_grid_anchors:')
            self._anchor_generator = \
                grid_anchor_3d_generator.GridAnchor3dGenerator()
        else:
            print('[rpn models.py] dont use_grid_anchors, using pred anchors: {}'.format(self._pred_anchors_dir))
            self._anchor_generator = \
                pred_anchor_3d_generator.PredAnchor3dGenerator()
        ...
        if len(self.dataset.classes) > 1:
            for class_idx in range(len(self.dataset.classes)):
                ...
                if use_grid_anchors:
                    ...
                    pred_anchor_boxes_3d = self._anchor_generator.generate(
                        sample_name=sample_name, 
                        class_name=self.dataset.classes[class_idx],
                        pred_anchors_dir=self._pred_anchors_dir)
                    all_anchor_boxes_3d.append(pred_anchor_boxes_3d)
            ...
            if use_grid_anchors:
                ...
                pred_anchor_boxes_3d = self._anchor_generator.generate(
                    ...
                    class_name=self.dataset.classes[class_idx],
                    pred_anchors_dir=self._pred_anchors_dir)
                all_anchor_boxes_3d = pred_anchor_boxes_3d

        #print('[rpn_models.py] fill_anchor_pl, all_anchor_boxes_3d.shape: {}'.format(all_anchor_boxes_3d.shape))
